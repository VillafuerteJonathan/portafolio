...
>
    ...
    <div
        class="bg-[#141c2f] p-6 w-full max-w-lg rounded-2xl border border-purple-500/40 
               shadow-purple-900/40 shadow-xl animate-scaleIn relative overflow-hidden"
    ...
    >
        ...
        <div class="relative z-10 grid grid-cols-2 gap-4">
            ...
            {#each tech as item}
                <div
                    role="button"
                    tabindex="0"
                    on:keydown={(e) => (e.key === "Enter" || e.key === " ") && null}
                    class="bg-[#1b263b] p-4 rounded-xl border border-white/10 flex flex-col items-center gap-3 
                           cursor-pointer transform transition-all duration-300 
                           hover:scale-[1.07] hover:-translate-y-1 hover:shadow-2xl hover:shadow-purple-600/40 group"
                    style="
                        transform:
        ...
        <button
            on:click={handleClose}
            class="mt-6 w-full py-2 bg-purple-600 hover:bg-purple-700 rounded-xl font-semibold 
                   text-white shadow-lg shadow-purple-700/30 transition-all hover:scale-105 relative z-10"
        >
            Cerrar
        </button>
    </div>
